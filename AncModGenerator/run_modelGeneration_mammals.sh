...
### Extract Anc238 sequence
hal_file="HAL/241-mammalian-2020v2.hal" # pathway to file
species_name="fullTreeAnc238" # 
species2="fullTreeAnc239" # most ancestal species
out_dir="model_"$species2 # Output directory (will be generated if it does not exist)
infile=$out_dir"/"LO_Anc239.bed"
final_file=$out_dir"/"AncRepeats_Allfam.bed"
list_ancestor="fullTreeAnc5 fullTreeAnc13 fullTreeAnc115 fullTreeAnc237"
...
RepeatMasker $out_file # Output will be in folder with the smae name than ref.
...
LOver_out=$out_dir"/"LO_"$species2".bed"
halLiftOver $hal_file $species_name $rmskOUTBEDF $species2 $out_dir"/"LO_Anc239.bed" --outPSL --noDupes

sort -k1,1 -k2,2n $out_dir"/"LO_Anc239.bed" | bedtools merge -i - > $out_dir"/"LO_Anc239.bed.temp" && mv $out_dir"/"LO_Anc239.bed.temp" $out_dir"/"LO_Anc239.bed"
